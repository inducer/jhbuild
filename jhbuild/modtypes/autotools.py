...
class AutogenModule(Package):
    ...
    "autogen.sh" script and the GNU build tools.'''
    type = 'autogen'
    ...
    def skip_configure(self, buildscript, last_state):
        ...
        return (os.path.exists(os.path.join(builddir, self.makefile)) and
                not buildscript.config.alwaysautogen and
                not self.alwaysautogen)
    ...
    def do_configure(self, buildscript):
        ...
        if buildscript.config.use_lib64 and self.standardautogen:
            cmd += " --libdir='${exec_prefix}/lib64'"
        cmd += ' %s' % self.autogenargs
        if self.standardautogen:
            cmd += ' ' + buildscript.config.module_autogenargs.get(id, 
        ...
        if self.autogen_sh == 'configure' and self.standardautogen:
    ...
    def do_clean(self, buildscript):
        ...
        cmd = '%s %s clean' % (self.makecommand, self.makeargs)
    ...
    def do_build(self, buildscript):
        ...
        cmd = '%s %s' % (self.makecommand, self.makeargs)
        buildscript.execute(cmd, cwd=self.get_builddir(buildscript))
    ...
    def do_check(self, buildscript):
        ...
        cmd = '%s %s check' % (self.makecommand, self.makeargs)
        buildscript.execute(cmd, cwd=self.get_builddir(buildscript))
    ...
    def do_dist(self, buildscript):
        ...
        if buildscript.config.makedistcheck:
            cmd = '%s %s distcheck' % (self.makecommand, self.makeargs)
        else:
            cmd = '%s %s dist' % (self.makecommand, self.makeargs)
        buildscript.execute(cmd, cwd=self.get_builddir(buildscript))
    ...
    def do_install(self, buildscript):
        if not self.skipinstall:
            buildscript.set_action('Installing', self)
            cmd = '%s %s install' % (self.makecommand, self.makeargs)
            buildscript.execute(cmd, cwd=self.get_builddir(buildscript))
